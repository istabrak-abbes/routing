...
class Pointer(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden, last_hh):
        ...
        print("rnnout",rnn_out)
        print("lasthh",last_hh)
        rnn_out = rnn_out.squeeze(1)
        print("rnnout1",rnn_out)
        # Always apply dropout on the RNN output
        rnn_out = self.drop_rnn(rnn_out)
        print("rnnout after dropout",rnn_out)
        if self.num_layers == 1:
            # If > 1 layer dropout is already applied
            last_hh = self.drop_hh(last_hh) 
            print("lasthh1",last_hh)
...
class DRL4TSP(nn.Module):
    ...
    def forward(self, static, dynamic, decoder_input=None, last_hh=None):
        ...
        print("static_hidden",static_hidden)
        dynamic_hidden = self.dynamic_encoder(dynamic)
        print("dynamic_hidden",dynamic_hidden)
        ...
        for _ in range(max_steps):
            ...
            print("logp first : ",tour_logp)
            tour_idx.append(ptr.data.unsqueeze(1))
            print("tour indices : ",tour_idx)
